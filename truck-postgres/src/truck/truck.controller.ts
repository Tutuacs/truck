...
export class TruckController {
  ...
  listByUserId(@Param('id') userId: string, @UserAuth() user: {role: number, id: string}) {
    return this.truckService.listByUserId(userId, user);
  }

  @Post('link-truck')
  linkTruck(@Body() trucks: AddTruckDto, @UserAuth() user: {userId: string}) {
    return this.truckService.linkTruck(trucks, user);
  }

  @Post('unlink-truck')
  unlinkTruck(@Body() trucks: AddTruckDto, @UserAuth() user: {userId: string}) {
